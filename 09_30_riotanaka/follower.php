...
//２．データ取得SQL作成
$stmt = $pdo->prepare("SELECT * FROM twitter_account JOIN following on twitter_account.username = following.followed WHERE username=:username ORDER BY follow_id DESC");
$stmt->bindValue(':username', $username,PDO::PARAM_STR);
$status = $stmt->execute();

// フォローされているアカウントを配列にIN!
$followd_by=array();
...
if($status==false) {
  ...
  while( $result = $stmt->fetch(PDO::FETCH_ASSOC)){ 
    array_push($followd_by,$result["follower"]);
  }
}
...
for($i=0;$i<count($followd_by);$i++){
//２．データ取得SQL作成
$stmt = $pdo->prepare("SELECT * FROM twitter_account WHERE username=:username");
$stmt->bindValue(':username', $followd_by[$i],PDO::PARAM_STR);
$status = $stmt->execute();
        ...
        $account_info.="<div class='info'>";
            $account_info.="<div class='top'>";
                ...
                $account_info.="<form action=''>";
                    $account_info.="<input class='follow' type='submit' value='Followed'>";
                $account_info.="</form>";
            $account_info.="</div>";
