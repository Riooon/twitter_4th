...
//２．データ取得SQL作成
$stmt = $pdo->prepare("SELECT * FROM twitter_account JOIN following on twitter_account.username = following.follower WHERE username=:username ORDER BY follow_id DESC");
...
if($status==false) {
  ...
  while( $result = $stmt->fetch(PDO::FETCH_ASSOC)){ 
    array_push($following_accounts,$result["followed"]);
  }
}
...
$stmt->bindValue(':username', $following_accounts[$i],PDO::PARAM_STR);
$status = $stmt->execute();
...
<body>
    ...
    <div class="center">
        ...
        <div class="follows">
            ...
            <a href="follower.php?username=<?= $view["username"] ?>" class="off">Followers</a>
        </div>
